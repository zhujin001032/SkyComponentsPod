...
@interface SkySharingViewController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate>
...
@implementation SkySharingViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section)
    {
        ...
        case 3:
        {
            ...
            cell.textLabel.text = @"平台内容定制--朋友圈";
            cell.detailTextLabel.text = @"点击演示";
            break;
        }
        case 4:
        {
            ...
            cell.textLabel.text = @"截屏分享--Facebook";
            cell.detailTextLabel.text = @"点击演示";
            break;
        }
        case 5:
...
- (void)simplyShare
{
     ...
     **/
    __weak SkySharingViewController *theController = self;
    [self showLoadingView:YES];
    
    //创建分享参数
    NSMutableDictionary *shareParams = [NSMutableDictionary dictionary];
    
    NSArray* imageArray = @[[UIImage imageNamed:@"ad_accela"]];
    
    if (imageArray) {
        
        [shareParams SSDKSetupShareParamsByText:@"分享内容"
                                         images:imageArray
                                            url:[NSURL URLWithString:@"http://www.missionsky.com"]
                                          title:@"分享标题"
...
- (void)showShareEditor
{
    ...
    if (imageArray) {
        ...
        [ShareSDK showShareEditor:SSDKPlatformTypeSinaWeibo
               otherPlatformTypes:@[@(SSDKPlatformTypeFacebook),@(SSDKPlatformTypeWhatsApp),@(SSDKPlatformTypeSMS)]
                      shareParams:shareParams
              onShareStateChanged:^(SSDKResponseState state, SSDKPlatformType platformType, NSDictionary *userData, SSDKContentEntity *contentEntity, NSError *error, BOOL end)
         {
...
- (void)platShare
{
    ...
    if (imageArray) {
        ...
        [shareParams SSDKSetupSinaWeiboShareParamsByText:@"分享内容 http://www.missionsky.com"
                                                   title:@"分享标题"
                                                   image:[UIImage imageNamed:@"ad_accela"]
                                                     url:nil
                                                latitude:0
                                               longitude:0
                                                objectID:nil
        ...
        [shareParams SSDKSetupWeChatParamsByText:@"分享内容" title:@"分享标题" url:[NSURL URLWithString:@"http://www.missionsky.com"] thumbImage:[UIImage imageNamed:@"ad_accela"] image:[UIImage imageNamed:@"ad_accela"] musicFileURL:nil extInfo:nil fileData:nil emoticonData:nil type:SSDKContentTypeAuto forPlatformSubType:SSDKPlatformSubTypeWechatTimeline];
        //进行分享
...
- (void)screenShare
{
    ...
    [SSEShareHelper screenCaptureShare:^(SSDKImage *image, SSEShareHandler shareHandler) {
        ...
        {
            //如果无法取得屏幕截图则使用默认图片
            image = [[SSDKImage alloc] initWithImage:[UIImage imageNamed:@"ad_accela"] format:SSDKImageFormatJpeg settings:nil];
        }
        
        //构造分享参数
        NSMutableDictionary *shareParams = [NSMutableDictionary dictionary];
        [shareParams SSDKSetupShareParamsByText:@"截屏分享，更便捷，更方便。"
                                         images:@[image]
                                            url:nil
                                          title:nil
                                           type:SSDKContentTypeImage];
        ...
        if (shareHandler)
        {
            shareHandler (SSDKPlatformTypeFacebook, shareParams);
        }
    }
                        onStateChanged:^(SSDKResponseState state, NSDictionary *userData, SSDKContentEntity *contentEntity, NSError *error) {
    ...
    if (self.enableShakeShare)
    {
        ...
        [SSEShareHelper beginShakeShare:nil
                     ...
                     onWillShareHandler:^(SSEShareHandler shareHandler)
         {
             ...
             if (imageArray)
             {
                 [shareParams SSDKSetupSMSParamsByText:@"摇着摇着就可以分享出去了，分享就是便捷方便。" title:@"测试短信分享" images:imageArray attachments:nil recipients:nil type:(SSDKContentTypeImage)];
                 //显示等待界面
                 [theController showLoadingView:YES];
                 ...
                 if (shareHandler)
                 {
                     shareHandler (SSDKPlatformTypeSMS, shareParams);
                 }
             }
         }
                         onStateChanged:^(SSDKResponseState state, NSDictionary *userData, SSDKContentEntity *contentEntity, NSError *error) {
...
- (void)oneKeyShare
{
    ...
    if (imageArray)
    {
        ...
        //构造分享参数
        NSMutableDictionary *shareParams = [NSMutableDictionary dictionary];
        [shareParams SSDKSetupShareParamsByText:@"我在使用的一键分享。"
                                         images:imageArray
                                            url:nil
                                          title:nil
                                           type:SSDKContentTypeImage];
